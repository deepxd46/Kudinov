...
def open_algorithm_window():
    if messagebox.askyesno("Подтверждение", "Вы точно хотите открыть новое окно?"):
        ...
        # Фрейм для радио-кнопок выбора метода ввода
        input_method_frame = tk.Frame(algorithm_window)
        ...
        checkbox = tk.Checkbutton(algorithm_window, text="Ориентированный граф", variable=is_directed_var)
        checkbox.pack(pady=3)

        input_container_frame = tk.Frame(algorithm_window)
        ...
        steps_area = tk.Text(algorithm_window, height=12, width=70, font=("Arial", 10), bg="#f7f7f7")
        steps_area.pack(pady=10)

        tk.Label(algorithm_window, text="Результат обработки:", font=("Arial", 12)).pack(pady=5)
        output_area = tk.Text(algorithm_window, height=8, width=70, font=("Arial", 12))
        ...
        def find_scc():
            try:
                ...
                if method == "manual":
                    ...
                    edge_prob = edge_prob_entry.get()
                    if not num_nodes or not edge_prob:
...
def show_algorithm_description():
    description = (
        ...
        "Теперь программа поддерживает как ручной ввод графа, так и генерацию случайного графа."
    )
